function Wall(wall) {
	...
	var numWall = wall.length;
	var wallWidth_ = 0.3;
	var wallHeight = 7;
	var vertexOffset = 0;
	var nv = 0;
	for(var i = 0; i < numWall; ++i)nv += wall[i].length/3;
	
	this.vertices = new Float32Array((nv - numWall) * 20 * 3);
	for (var i=0; i<numWall; ++i) {
		for(var j = 1; j < wall[i].length / 3; ++j){
			if(wall[i][j * 3] == wall[i][(j - 1) * 3]){
				wallWidth = wallWidth_ * Math.abs(wall[i][j * 3 + 2] - wall[i][(j - 1) * 3 + 2])/(wall[i][(j - 1) * 3 + 2] - wall[i][j * 3 + 2]);
				this.vertices[vertexOffset + 0] = wall[i][j * 3] + wallWidth/2;//0
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3] + wallWidth/2;//1
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][j * 3] + wallWidth/2;//4
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3] + wallWidth/2;//5
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2];
				vertexOffset += 3;



				this.vertices[vertexOffset + 0] = wall[i][j * 3] - wallWidth/2;//2
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3] - wallWidth/2;//3
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][j * 3] - wallWidth/2;//6
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2];
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3] - wallWidth/2;//7
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2];
				vertexOffset += 3;
			...
			else if(wall[i][j * 3 + 2] == wall[i][(j - 1) * 3 + 2]){
				wallWidth = -wallWidth_ * Math.abs(wall[i][j * 3] - wall[i][(j - 1) * 3])/(wall[i][(j - 1) * 3] - wall[i][j * 3]);
				this.vertices[vertexOffset + 0] = wall[i][j * 3];//0
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2] + wallWidth/2;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3];//1
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2] + wallWidth/2;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][j * 3];//4
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2] + wallWidth/2;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3];//5
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2] + wallWidth/2;
				vertexOffset += 3;

				this.vertices[vertexOffset + 0] = wall[i][j * 3];//2
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2] - wallWidth/2;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3];//3
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1];
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2] - wallWidth/2;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][j * 3];//6
				this.vertices[vertexOffset + 1] = wall[i][j * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][j * 3 + 2] - wallWidth;
				vertexOffset += 3;
				this.vertices[vertexOffset + 0] = wall[i][(j - 1) * 3];//7
				this.vertices[vertexOffset + 1] = wall[i][(j - 1) * 3 + 1] + wallHeight;
				this.vertices[vertexOffset + 2] = wall[i][(j - 1) * 3 + 2] - wallWidth;
				vertexOffset += 3;
	...
	this.textureCoord = new Float32Array((nv - numWall) * 2 * 20);
	var d = 0.0;
	var scale = 0.1;
	...
	this.triangleIndices = new Uint16Array((nv - numWall) * 3 * 10);
	...
	this.numVertices  = (nv - numWall) * 20;
	this.numTriangles = (nv - numWall) * 10;
};
